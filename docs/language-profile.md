...
copyable structs with exact scalar or nested nominal struct fields and
authoritative target-Clang ABI signatures. Direct register coercion, expansion into multiple physical values,
...
nominal value. Nested definitions close transitively and lower in dependency
order without flattening nominal field identity. Field writes, unions, and
non-trivial ownership remain outside this slice.
