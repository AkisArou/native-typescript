...
| Native structs/by-value ABI | Direct/expanded/indirect slice implemented | Authoritative nominal layout and target-Clang physical signatures lower through C/LLVM for direct registers, expanded values, plain indirect pointers, `byval`, and `sret`; add nested aggregates, unions, pointer/out-parameter projection, and non-trivial values as real bindings require | scriptc fork + SCABI |
...
| Native declaration projection | Class construction slice implemented | External native declaration classes lower canonical `new Class()` construction and named static factories without materializing runtime class objects; add property accessors, enum/flag projection, and broader declaration forms only from authoritative binding semantics | scriptc fork + binding generators |
