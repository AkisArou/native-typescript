...
| Exact native integers | Partial systems capability | Fixed-width and pointer-sized integers, same-type wrapping `+`, `-`, and `*`, and exact same-storage `===`/`!==` are implemented through frontend, IR, C/LLVM, and ABI; generated GTK `gint` input/output and nominal flags comparison are real package gates; add the remaining arithmetic, bitwise, ordered-comparison, and explicit conversion families | scriptc fork |
