...
| Native C strings | First end-to-end slice implemented | SCABI distinguishes explicit byte spans from implicit-length NUL strings; C/LLVM borrow the runtime's terminated UTF-8 storage and reject embedded NUL before native entry; add transcoding and retained/copy modes only when a binding requires them | scriptc fork + SCABI |
