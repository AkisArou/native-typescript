...
| Exact native integers | Partial systems capability | Fixed-width and pointer-sized integers, same-type wrapping `+`, `-`, and `*`, exact-width `&`, `|`, and `^`, and exact same-storage `===`/`!==` are implemented through frontend, IR, C/LLVM, and ABI; generated GTK `gint` input/output, nominal flags comparison, and `Vertical | Horizontal == BothAxes` are real package gates; add division/remainder, shifts, unary operations, ordered comparisons, and explicit conversions only with defined exact-width semantics | scriptc fork |
...
| Native declaration projection | Constructors, properties, and named constants implemented | External native declarations lower canonical `new Class()` construction, named static factories, getter/setter properties, and exact enum/flags members without materializing runtime class or namespace objects; add broader declaration forms and typed flags composition only from authoritative binding semantics | scriptc fork + binding generators |
| Receiver-owned callbacks | Required lifecycle capability | The current retained-callback table is an external root and native handles are not collector-traced; add a traceable receiver-to-registration-to-closure edge with transactional attach, early cancellation, invalidation, and shutdown gates before GTK signals claim automatic receiver ownership | scriptc fork + SCABI + target runtime |
