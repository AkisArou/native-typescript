...
| Retained callbacks | Runtime ownership foundation implemented | Slot/generation tokens and the owner table provide exact admission/root lifetime; result native handles now order close → blocking native destructor → cancellation complete; add generated payload/invocation thunks, Native IR, and target-loop lowering | scriptc fork |
| Foreign-thread callbacks | Runtime gateway foundation implemented | The instance-owned MPSC queue, retained callback tokens, owner table, bounded drains, and shutdown races are implemented and sanitizer-tested; add copied ABI payload/invocation thunks and target-loop integration without foreign heap access | scriptc fork + target runtime |
