...
| Exact native integers | Partial systems capability | Fixed-width and pointer-sized integers, same-type wrapping `+`, `-`, and `*`, exact-width `&`, `|`, and `^`, exact same-storage `===`/`!==`, and all four orderings at the declared width and signedness are implemented through frontend, IR, C/LLVM, and ABI; nominal flags comparison and typed `combine(Vertical, Horizontal) == BothAxes` are real package gates; add division/remainder, shifts, unary operations, and explicit conversions only with defined exact-width semantics | scriptc fork |
