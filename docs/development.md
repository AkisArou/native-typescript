...
reached fixed- and pointer-width integer bindings plus its padded by-value
struct and owned opaque handle, resolves their TypeScript declaration symbols,
and links the resulting source-lowered programs against the permanent C
fixture through both ScriptC backends:
...
Run it whenever Native IR, SCABI scalar, aggregate, or handle bindings, either
backend, or the fixture's C implementation changes. The sanitizer form compiles
both the ScriptC runtime and fixture with AddressSanitizer and enables the
runtime reference-count audit.
