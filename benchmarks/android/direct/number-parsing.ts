...
/** Keep the three JavaScript numeric grammars separately observable. The
 * aggregate workload remains the application-shaped control; these probes
 * identify which parser owns a regression without changing its input mix. */
export function runParseIntWorkload(iterations: number): number {
  const inputs = [
    "0", "7", "42", "-17", "255", "1024", "6553", "-3276",
    "12345", "-7654", "2147", "-9999", "73", "8080", "-4096", "3141",
  ];
...
export function runParseFloatWorkload(iterations: number): number {
  const inputs = [
    "0.5", "-2.25", "3.125", "1e3", "-0.03125", "42.75", "512.5", "-128.125",
    "0.125", "64.875", "-16.5", "2048.25", "-4096.75", "7.5", "0e0", "123.375",
  ];
...
export function runNumberFromStringWorkload(iterations: number): number {
  const inputs = [
    "1.25", "-3.5", "6.125", "2.5e2", "-0.0625", "18.75", "256.25", "-64.5",
    "0.375", "32.625", "-8.25", "1024.5", "-2048.125", "15.875", "0.0", "61.25",
  ];
