...
export function generateJvmAdapterSource(
  ...
  const lines = [
    ...
    ...(usesStrings
      ? [
          `enum { ${prefix}_inline_utf16_units = 256 };`,
          "",
          ...
          " * Ordinary short arguments stage UTF-16 in this native frame; a",
          " * larger argument takes one heap fallback. Results borrow JNI's",
          " * UTF-16 view and allocate only their final UTF-8 owner. */",
          `static jstring ${prefix}_utf8_to_jstring(JNIEnv *env,`,
          "                                         const char *utf8,",
          "                                         char **error) {",
          "  size_t byteLength = strlen(utf8);",
          `  jchar inlineUnits[${prefix}_inline_utf16_units];`,
          "  jchar *units = inlineUnits;",
          `  if (byteLength > ${prefix}_inline_utf16_units) {`,
          "    if (byteLength > SIZE_MAX / sizeof(jchar)) {",
          `      fprintf(stderr, "${prefix}: string is too large to bridge\\n");`,
          "      abort();",
          "    }",
          "    units = (jchar *)malloc(byteLength * sizeof(jchar));",
          "    if (units == NULL) {",
          ...
          "    if (units != inlineUnits) free(units);",
          "    if ((*env)->ExceptionCheck(env)) {",
          `      ${prefix}_capture(env, error);`,
          "      return NULL;",
          "    }",
          ...
          "  if ((size_t)unitCount > (SIZE_MAX - 1) / 4) {",
          `    fprintf(stderr, "${prefix}: Java string is too large to bridge\\n");`,
          "    abort();",
          "  }",
          ...
          `      ${prefix}_capture(env, error);`,
          "    } else {",
          `      *error = ${prefix}_message("GetStringChars failed");`,
          "    }",
          "    return NULL;",
          "  }",
          "  size_t written = 0;",
          ...
          "      (*env)->ReleaseStringChars(env, string, units);",
          "      free(bytes);",
          `      *error = ${prefix}_message(`,
