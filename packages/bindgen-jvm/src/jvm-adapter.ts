...
export interface JvmClassReleaseAdapter {
  readonly className: string;
  /** Releases one stable reference of exactly this class; the compiler
   * requires a destructor to consume the type it releases, so each class
   * gets its own spelling of the one release. */
  readonly adapterSymbol: string;
}
...
export interface JvmAdapterSource {
  ...
  readonly classReleases: readonly JvmClassReleaseAdapter[];
...
> = Object.freeze({
  ...
  classReleases: {
    ...
    custom:
      ...
      "the release is unary, which is what destructor-as-data needs, and " +
      "each class gets its own spelling because a destructor consumes " +
      "exactly the type it releases. Owner-confined destruction already " +
...
export function generateJvmAdapterSource(
  ...
  const classReleases = snapshot.classes.map((class_) =>
    Object.freeze({
      className: class_.binaryName,
      adapterSymbol: `${prefix}_release_${cSafe(class_.binaryName)}`,
    })
  );
  const lines = [
    ...
    ...(usesStrings
      ...
      : []),
    `static void ${prefix}_release_ref(void *ref) {`,
    "  JNIEnv *env = NULL;",
    `  if (${prefix}_vm == NULL ||`,
    `      (*${prefix}_vm)->GetEnv(${prefix}_vm, (void **)&env,`,
    "                              JNI_VERSION_10) != JNI_OK) {",
    ...
    "}",
    "",
    ...classReleases.flatMap(({ className, adapterSymbol }) => [
      `/* releases one ${className} reference */`,
      `void ${adapterSymbol}(void *ref) {`,
      `  ${prefix}_release_ref(ref);`,
      "}",
      "",
    ]),
  ...
  const header = [
    ...
    `jint ${bindSymbol}(JNIEnv *env, char **error);`,
    `jint ${bindVmSymbol}(JavaVM *vm, char **error);`,
    ...classReleases.map(({ adapterSymbol }) => `void ${adapterSymbol}(void *ref);`),
    `const char *${errorMessageSymbol}(void *error);`,
    `void ${errorReleaseSymbol}(void *error);`,
    ...headerDeclarations,
    "",
