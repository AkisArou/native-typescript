...
export function generateJvmSubclassSource(
  ...
  /* A class is EXTENDED and an interface is IMPLEMENTED, and the two
   * differ in more than the keyword: an interface has no constructor to
   * inherit and its abstract members have no base implementation to
   * reach, so neither the no-arg rule nor the super binding applies. What
   * they share is the reason the generator exists — a foreign caller
   * dispatching virtually into a TypeScript handler. */
  const implementsInterface = base.kind === "interface";
  if (base.kind !== "class" && base.kind !== "interface") {
    diagnostics.push(diagnostic(
      path,
      `'${base.binaryName}' is a ${base.kind}, which is neither a class to ` +
        "extend nor an interface to implement",
    ));
  }
   ...
   * the base must offer an accessible no-arg one. Pass-through
   * constructors are their own slice with their own demand. An INTERFACE
   * has no constructor to inherit, so the rule does not apply to one. */
  ...
  for (const overrideSelection of selection.overrides) {
    ...
    if (implementsInterface && !method.access.abstract) {
      diagnostics.push(diagnostic(
        overridePath,
        `'${name}' is a default method: implementing it here would replace ` +
          "an implementation the interface already provides, and reaching " +
          "that one is a base call an interface does not offer through " +
          "invokespecial from a generated class — its own slice",
      ));
      continue;
    }
    ...
    /* An abstract interface member has no implementation to reach, so
     * there is nothing for a base call to name. The generated class is
     * the first implementation rather than a replacement for one. */
    const hasBaseImplementation = !implementsInterface;
  ...
  const source = [
    ...
    `public final class ${subclassSimpleName} ${
      implementsInterface ? "implements" : "extends"
    } ${baseSourceName} {`,
