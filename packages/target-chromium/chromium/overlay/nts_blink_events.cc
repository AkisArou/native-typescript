...
NtsWebStatus DecodeAtomicString(NtsUtf8View source, blink::AtomicString* out) {
  if (!out || (!source.data && source.length != 0)) {
    return NTS_WEB_INVALID_ARGUMENT;
  }
  const auto bytes = UNSAFE_BUFFERS(base::span(source.data, source.length));
  blink::String string = blink::String::FromUtf8(bytes);
...
extern "C" NtsWebSubscriptionResult nts_web_event_target_add_event_listener(
  ...
  if (realm_status != NTS_WEB_OK) {
    return Failure(realm_status);
  }
  if (callback.generation == 0) {
    return Failure(NTS_WEB_INVALID_ARGUMENT);
  }
  ...
  auto* listener =
      blink::MakeGarbageCollected<nts::blink_bridge::BlinkNativeEventListener>(
          realm, callback);
