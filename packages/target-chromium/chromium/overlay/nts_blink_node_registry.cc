...
BlinkNodeRegistry::BlinkNodeRegistry(uint64_t realm) {
...
NtsWebStatus BlinkNodeRegistry::Intern(blink::Node* node,
  ...
  if (BlinkNodeRoot* existing = FindRoot(node, &existing_slot)) {
    (void)existing;
    const auto slots =
        UNSAFE_BUFFERS(base::span(table_.slots, table_.slot_count));
    NtsWebHandle handle = {
        .realm = table_.realm,
        .slot = existing_slot,
        .generation = slots[existing_slot].generation,
    };
...
WebTypeId BlinkNodeRegistry::TypeOf(const blink::Node& node) {
  ...
  if (blink::IsA<blink::HTMLElement>(node)) {
    return WebTypeId::kHTMLElement;
  }
  if (blink::IsA<blink::Element>(node)) {
    return WebTypeId::kElement;
  }
  if (blink::IsA<blink::Document>(node)) {
    return WebTypeId::kDocument;
  }
...
BlinkNodeRoot* BlinkNodeRegistry::FindRoot(blink::Node* node,
  ...
  for (size_t index = 0; index < slots.size(); ++index) {
    const NtsHandleSlot& slot = slots[index];
    if (!slot.occupied) {
      continue;
    }
    ...
    if (out_slot) {
      *out_slot = static_cast<uint32_t>(index);
    }
    return root;
  }
  return nullptr;
